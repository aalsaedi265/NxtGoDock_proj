...
function UserInterface({ backendName }: UserInterfaceProps) {
  ...
  async function createUser(e: React.FormEvent<HTMLFormElement>) {
      e.preventDefault();
// Send a POST request to create a new user with the data from newUser at the specified apiUrl
      try {
  ...
  return (
    <div className={`user-interface ${bgColor} ${backendName} w-full max-w-md p-4 my-4 rounded shadow`}>
       <Image 
        ...
        width={80} // for w-20, considering 1rem = 4px
        height={80} // for h-20, considering 1rem = 4px
        className="mb-6 mx-auto" // Tailwind CSS classes for margin
      />
      <h2
